...
net=read.net('data/networks/BEWARE_v5_learn_pt1.net', debug = T)
...
df.var[df.var$fishing_style=='trawler',]$Freq=c(1,0)
df.var[df.var$fishing_style%in% c('recreational', 'archipelago'),]$Freq=c(0,1)
df.var[df.var$fishing_style %in% c('guide','coastal'),]$Freq=c(0.5,0.5)
...
bnlearn::write.net( 'data/networks/BEWARE_v5_learn_pt2.net', net)
