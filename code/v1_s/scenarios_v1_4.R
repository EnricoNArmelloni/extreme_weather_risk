...
scen.grid2=expand.grid(stock_status=ss.list)

ev.store=NULL
for(i in 1:nrow(scen.grid2)){
  
  i.res=cpdist(net.s5, nodes = c('economic_risk', 'societal_risk','individual_risk'), 
               evidence = (  stock_status==as.character(scen.grid2[i,])))
  
 
  i.res$stock_status=as.character(scen.grid2[i,])
  
  i.res=i.res%>%
    pivot_longer(-c( stock_status), names_to = 'node', values_to = 'est')%>%
    dplyr::group_by( stock_status, node,est)%>%
    tally()%>%
    dplyr::group_by( stock_status, node)%>%
    dplyr::mutate(prob=n/sum(n))
  ev.store=rbind(ev.store, i.res)
...
p.f2=ev.store%>%
  ...
  #dplyr::filter(est!='not_relevant')%>%
  dplyr::mutate(est=factor(est, levels=c('Not Relevant', 'Low', 'Medium',  'High')))%>%
  ggplot(aes(x=stock_status, y=prob, fill=est))+
