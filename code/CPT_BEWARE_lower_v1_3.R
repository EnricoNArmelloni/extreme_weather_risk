...
for(i in 1:length(risk.vars)){
  ...
  if(x.var=='economic_risk'){
    ...
    l.w=c(2,1,1,1)
    xdim2=dim(array.var)[1:4]
    xnam2=dimnames(array.var)[1:4]
  ...
  supp.data=data.frame(node=c(target.dims,x.var), 
                       ...
                       direction=direction,
                       id=1:length(xdim2)) 
  x.cpt=rank.cpt(nodes.df = supp.data, uncertainty = 0.05, algorithm = 'max', xnam=xnam2)
    ...
    if(i==3){
      ...
      z.cpt= x.cpt[[1]]%>%
        ...
        arrange(desc(innovative_capacity), desc(profit))
      df.var[df.var$economic_risk=='not_relevant',]$Freq=0
      df.var[df.var$strategy=='not_relevant',]$Freq=c(0,0,0,1)
      df.var[df.var$strategy!='not_relevant' & df.var$economic_risk!='not_relevant',]$Freq=z.cpt$value
      net[[x.var]]=array(df.var$Freq, dim=xdim, dimnames=xnam)
    }
}
