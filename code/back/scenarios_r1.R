...
ch=plot.event%>%
  ...
  dplyr::group_by(fishing_style, event, node, stock_status)%>%
  dplyr::summarise(risk.score=sum(risk.score))


pl.temp=ch%>%
  dplyr::filter(stock_status=='status_quo')%>%
  ggplot(aes(x=fishing_style, y=event, fill=risk.score))+
...
ggsave(plot=pl.temp, 'results/scenarios/scen1_temp.png', width = 30, height = 12, units='cm', dpi=500)
