...
# DAG
net=read.net('data/networks/BEWARE_v1_2.net', debug = T)
...
for(z in 1:length(styles$f.style)){
  ...
  for(j in 1:length(i.evts)){
      ...
      if(z.style$season!=evt.season){
        if(evt.season!='all'){
          df.var[df.var$event==j.event & df.var$fishing_style==styles[z,]$f.style &
                 df.var$flexibility=='status_quo',]$Freq=c(0,0,0,1)
        next 
        }
      }
    }
    ...
    if(nrow(j.answ)==0){
      df.var[df.var$event==j.event & df.var$fishing_style==styles[z,]$f.style &
             df.var$flexibility=='status_quo',]$Freq=c(1,0,0,0)
      next
    }
    ...
    if(length(j.strategy.unique)>1){
      ...
      df.var[df.var$event==j.event & 
               ...
               df.var$flexibility=='status_quo',]$Freq=c(pARC$p,0)
      next  
    }
    ...
    if(j.strategy.unique=='A'){
      ...
      i.cpt=answer.to.cpt(x.ans=j.answ$value, 
                          ...
                          x.range=c(answ.range$min, answ.range$max),
                             dim.labs = c('cope', 'adapt'))
      df.var[df.var$event==j.event & df.var$fishing_style==styles[z,]$f.style &
               df.var$flexibility=='status_quo',]$Freq=c(i.cpt[1],0,i.cpt[2],0)
      next 
    }
    if(j.strategy.unique =='R'){
      ...
      i.cpt=answer.to.cpt(x.ans=j.answ$value, 
                             ...
                             dim.labs = c('cope', 'react'))
      df.var[df.var$event==j.event & df.var$fishing_style==styles[z,]$f.style &
               df.var$flexibility=='status_quo',]$Freq=c(i.cpt,0,0)
      next
    }
...
df.var[df.var$flexibility=='flexible',]$Freq=c(0,0.5,0.5,0)
df.var[df.var$flexibility=='rigid',]$Freq=c(1,0,0,0)
net[[i.node]]=array(df.var$Freq, dim=xdim, dimnames=xnam)
...
target.dims=names(df.var)
target.dims=target.dims[-which(target.dims %in% c(i.node,'Freq'))]
df.var[df.var$strategy=='not_relevant',]$Freq=c(rep(0,7),1)
...
df.var[df.var$strategy!='not_relevant' & df.var$fishing_style=='trawler',]$Freq=c(1, rep(0,7))
df.var[df.var$strategy!='not_relevant' & df.var$fishing_style=='archipelago',]$Freq=c(0,0.5,0.5, rep(0,5))
df.var[df.var$strategy!='not_relevant' & df.var$fishing_style=='recreational',]$Freq=c(0,0,0,0,1,0,0,0)

# those that changes, set baseline first. 
df.var[df.var$strategy!='not_relevant' & df.var$fishing_style=='recreational',]$Freq=c(0,0,0,0,0.42,0,0.58,0) # tabell 2. FIshing days inland = 5814; Kust = 4303. Kust accounts for 42% of rec fishing
df.var[df.var$strategy!='not_relevant' & df.var$fishing_style=='guide',]$Freq=c(0,0,0,0,0.42,0,0.58,0) # tabell 2. FIshing days inland = 5814; Kust = 4303. Kust accounts for 42% of rec fishing
coast.fdays
df.var[df.var$strategy!='not_relevant' & df.var$fishing_style=='coastal',]$Freq=c(0,0.3,0.489,0.21,0,0,0,0)
...
df.var[df.var$strategy=='adapt' & 
         ...
         df.var$event=='abl',]$Freq=c(0,0,0,0,0,0,1,0) # goes inland
...
df.var[df.var$strategy=='adapt' & 
         ...
         df.var$event=='hws',]$Freq=c(0,0.65,0,0.35,0,0,0,0) # less herring, more perch and salmon

## winter
df.var[df.var$strategy!='not_relevant' & 
         df.var$fishing_style%in%c('guide', 'recreational')&
         df.var$event%in%c('hww', 'ici'),]$Freq=c(0,0,0,0,0,1,0,0)

df.var[df.var$strategy=='adapt' & df.var$fishing_style=='guide'&
         df.var$event=='hww',]$Freq=c(0,0,0,0,1,0,0,0) # rod
...
bnlearn::write.net( 'data/networks/BEWARE_learn_pt1.net', net)
