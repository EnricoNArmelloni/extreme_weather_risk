...
safety.cpt=expand.probs(x.levels=list(c('no','Minor','Major'), c('yes','no')) ,
...
# convert to df and save dimnames to convert it back
ch=as.array(net$catchability)
ch2=ch[['prob']]
xdim=dim(ch2)
xnam=dimnames(ch2)
ch3=as.data.frame(ch2)

x.lev=levels(ch3$catchability)
target.dims=names(ch3)
target.dims=target.dims[-which(target.dims %in% c('catchability','Freq'))]

# extract the answer
x.answ=dat[dat$short_description=='catchability',]
# filter on the events
x.answ=x.answ[x.answ$event_code %in% names(ch3),] 
# check on target
if(is.na(unique(x.answ$target))){
  x.answ$target='not_specified'
}
# let's make this simple: do it for one event and for one species


x.new.cpt=answer.to.cpt(x.ans=x.answ[x.answ$event_code=='hws',]$value, 
              unc = x.answ[x.answ$event_code=='hws',]$uncertainty, 
              div.factor = 5,
              dim.name='x', 
              dim.labs = x.lev)
ch3[ch3$hws=='yes' & ch3$gal=='no'& ch3$abl=='no',]$Freq=x.new.cpt # on the specific event


x.new.cpt=answer.to.cpt(x.ans=x.answ[x.answ$event_code=='gal',]$value, 
                        unc = x.answ[x.answ$event_code=='gal',]$uncertainty, 
                        div.factor = 5,
                        dim.name='x', 
                        dim.labs = x.lev)
ch3[ch3$hws=='no' & ch3$gal=='yes'& ch3$abl=='no',]$Freq=x.new.cpt # if you want hardcore separation. But this is an important aspect to consider
...
supp.data=data.frame(node=c('catch_condition',  'damage', 'personal_safety','stress'), 
                     link.w=c(x.answ$value[c(3,2,1)], 0),
                     states=c(3,4,3,2),
                     type=c('parent','parent','parent','child'),
                     direction=c('neg', 'pos','neg', 'neg'),
                     id=1:4)
x.cpt=get.cpt(nodes.df = supp.data, uncertainty = unique(x.answ$uncertainty), algorithm = 'min', xnam=xnam)
net[[x.var]]=x.cpt$bn.cpt
...
ch3=as.data.frame(ch2)
summary(ch3)
ch4=array(ch3$Freq, dim=xdim, dimnames=xnam)
ch2==ch4
...
x.ans=dat[dat$short_description=='catchability',]
