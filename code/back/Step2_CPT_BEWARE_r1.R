...
for(z in 1:length(styles$f.style)){
  ...
  i.relevance=evt.relevance[evt.relevance$code==z.style$f.style, ]
  ...
  for(j in 1:length(i.evts)){
    ...
    if(j.event %in% i.relevance$event_code){
      ...
      df.var[df.var$extreme_event==j.event & df.var$fishing_style==styles[z,]$short_description ,]$Freq=c(0,0,0,1)
      next 
    }
    # if so, go ahead
    ...
    if(nrow(j.answ)==0){
      df.var[df.var$extreme_event==j.event & df.var$fishing_style==styles[z,]$short_description ,]$Freq=c(1,0,0,0)
      next
    }
    ...
    if(length(j.strategy.unique)>1){
      if('A' %in% j.strategy.unique){
       ...
       if(nrow(j.adapt)>1){
         j.adapt=uncertainty.function(j.adapt)
        if(nrow(j.adapt)>1){
        ...
        p=answer.to.cpt(x.ans=mean(j.adapt$value.norm), 
                            unc=mean(j.adapt$unc.norm),
                           dim.name = i.node,
      ...
      if(nrow(j.react)>1){
        if(unique(j.react$id_q) %in% 1:2){
         ...
         j.react=uncertainty.function(j.react)
        }
      }else{
        j.react$weight=1
      }
        p=answer.to.cpt(x.ans=mean(j.react$value.norm), 
                        unc=mean(j.react$unc.norm),
                        dim.name = i.node,
      ...
      df.var[df.var$extreme_event==j.event & 
               df.var$fishing_style==styles[z,]$short_description ,]$Freq=c(pARC$p,0)
      next  
    }
    ...
    i.cpt=answer.to.cpt(x.ans=j.answ$value.norm, 
                             unc=j.answ$unc.norm,
                             dim.name = i.node,
                             dim.labs = c('cope', 'other'))
    if(j.strategy.unique=='A'){
      # adapt or react? 
      df.var[df.var$extreme_event==j.event & df.var$fishing_style==styles[z,]$short_description ,]$Freq=c(i.cpt[1],0,i.cpt[2],0)
      next 
    }
    if(j.strategy.unique =='R'){
      # cope or react? 
      df.var[df.var$extreme_event==j.event & df.var$fishing_style==styles[z,]$short_description ,]$Freq=c(i.cpt,0,0)
      next
    }
...
df.var[df.var$strategy_to_change=='cope',]$Freq=c(1,0)
...
for(z in 1:length(styles$f.style)){
  ...
  if(nrow(j.answ)==0){
    df.var[df.var$extreme_event==j.event & 
             ...
             df.var$strategy_to_change=='cope',]$Freq=c(1,0)
    next
  }
  
  if(nrow(j.answ)>1){
    if(z.style$f.style%in% c('rcf')){
      j.answ=j.answ%>%
        arrange(desc(value.norm))
      j.answ=j.answ[1,]
    }else{
    j.answ=uncertainty.function(j.answ)
  }
  ...
  i.cpt=answer.to.cpt(x.ans=j.answ$value.norm, 
                      unc=j.answ$unc.norm*0.25,
                      dim.name = i.node,
                      dim.labs = c('no','yes'))
  df.var[df.var$extreme_event==j.event & 
           df.var$fishing_style==styles[z,]$short_description & 
           df.var$strategy_to_change=='cope',]$Freq=c(i.cpt)
  }
}
...
df.var[df.var$strategy_to_change=='adapt' & df.var$fishing_style=='guide'&
