...
#array.var[c(3,7)]=0
net.s1[['extreme_event']]=array.var
...
for(i in 1:length(ev.list)){
  i.res=cpdist(net.s1, nodes = c('catch_condition', 'personal_safety', 'damage', 'catchability', 'go_fishing'), 
...
p.f1=ev.store%>%
  ...
  ggplot(aes(x=extreme_event, y=prob, fill=est))+
...
p.f1=ev.store%>%
  dplyr::filter(node %in% c('personal_safety', 'damage'))%>%
  #dplyr::filter(est!='not_relevant')%>%
  replace(is.na(.),0)%>%
  #dplyr::mutate(prob=prob*(1-notrel))%>%
  ggplot(aes(x=extreme_event, y=prob, fill=est))+
...
p.f1=ev.store%>%
  dplyr::filter(node %in% c('catch_condition', 'catchability'))%>%
  #dplyr::filter(est!='not_relevant')%>%
  replace(is.na(.),0)%>%
  #dplyr::mutate(prob=prob*(1-notrel))%>%
  ggplot(aes(x=extreme_event, y=prob, fill=est))+
...
scen.grid2=expand.grid(fishing_style=fi.list, extreme_event=ev.list, stock_status=ma.list)
ev.store=NULL
for(i in 1:nrow(scen.grid2)){
  ...
  i.res=cpdist(net.s2, nodes = c('economic_risk', 'societal_risk','individual_risk'), 
               evidence = list(fishing_style = as.character(scen.grid2[i,]$fishing_style),
                               extreme_event=as.character(scen.grid2[i,]$extreme_event) ,
                             stock_status=as.character(scen.grid2[i,]$stock_status)), 
               n=10^5, method='lw')
  
  i.res$fishing_style=as.character(scen.grid2[i,]$fishing_style)
  i.res$extreme_event=as.character(scen.grid2[i,]$extreme_event)
  i.res$stock_status=as.character(scen.grid2[i,]$stock_status)
  
  i.res=i.res%>%
    pivot_longer(-c(fishing_style, extreme_event, stock_status), names_to = 'node', values_to = 'est')%>%
    dplyr::group_by(fishing_style, extreme_event, stock_status, node,est)%>%
    tally()%>%
    dplyr::group_by(fishing_style, extreme_event, stock_status, node)%>%
    dplyr::mutate(prob=n/sum(n))
  ev.store=rbind(ev.store, i.res)
...
excl=ev.store[ev.store$est =='not_relevant',]%>%
  ungroup()%>%
  distinct(fishing_style, extreme_event)

plot.event=ev.store[ev.store$est!='not_relevant',]
plot.event=plot.event%>%
  dplyr::group_by(extreme_event, fishing_style, stock_status, node)%>%
  dplyr::mutate(prob=prob/sum(prob))



plot1c=plot.event%>%
  dplyr::mutate(risk.score=ifelse(est=='high',1,ifelse(est=='medium', 0.5,0)),
                risk.score=risk.score*prob)%>%
  dplyr::group_by(fishing_style, extreme_event, node, stock_status)%>%
  dplyr::summarise(risk.score=sum(risk.score))
...
pl.1c=plot1c%>%
  ...
  ggplot(aes(x=fishing_style, y=extreme_event, fill=risk.score))+
  ...
  theme(legend.position = 'bottom')+
  labs(fill='Risk score')+
  ylab('MEW')+
  xlab('Fishing Style')

p.f5=ggpubr::ggarrange(p.f1, p.f2, pl.1c, nrow=3,  labels = c('a)', 'b)', 'c)'))

ggsave(plot=pl.1c, 'results/scenarios/scen1_temp.png', width = 30, height = 12, units='cm', dpi=500)
...
base.grid=expand.grid(fishing_style=unique(plot.event$fishing_style), 
                      extreme_event=unique(plot.event$extreme_event), 
...
plot.event[which(paste0(plot.event$fishing_style, plot.event$extreme_event) %in% paste0(excl$fishing_style, excl$extreme_event)),]$prob=-999
plot.event[plot.event$prob<0,]$prob=NA
ev.val=mean(plot.event[!is.na(plot.event$prob),]$prob)


plot1c%>%
  pivot_wider(names_from = stock_status, values_from = risk.score)%>%
  dplyr::mutate(worst=worst-status_quo, 
                better=better-status_quo)%>%
  pivot_longer(cols = c(worst, better), values_to = 'risk.score', names_to = 'stock_status')%>%
  #na.omit()%>%
  dplyr::filter(stock_status!='status_quo')
...
plot.event=plot.event%>%
  dplyr::mutate(fishing_style=ifelse(fishing_style=='recreational',
                                     'rcf', ifelse(fishing_style=='small_scale', 'ssf',
                                                   ifelse(fishing_style=='coastal_salmon' , 'sal' ,
                                                          ifelse(fishing_style=='household' ,'arc',
                                                                 ifelse(fishing_style=='trawler' ,'trw','fgu'))))))%>%
...
p.f4=plot.event%>%
  ...
  ggplot(aes(x=fishing_style, y=extreme_event, fill=prob))+
  geom_tile(color='black')+
  facet_grid(cols=vars(node), rows=vars(stock_status))+
  scale_fill_gradient2(midpoint=0.5, low='lightgreen', high='red', mid='yellow',na.value = "grey98")+
  theme(legend.position = 'bottom')+
...
pl.f3=pl.diff%>%
  #dplyr::filter(abs(prob)>0.05)%>%
  dplyr::filter(node!='economic_risk')%>%
  ggplot(aes(x=extreme_event, y=prob, fill=prob))+
...
##
#p.f5=ggpubr::ggarrange(p.f1, p.f2, pl.temp, nrow=3,  labels = c('a)', 'b)', 'c)'))
#ggsave(plot=p.f5, 'results/scenarios/scen1_2.png', width = 18, height = 21, units='cm', dpi=500)
