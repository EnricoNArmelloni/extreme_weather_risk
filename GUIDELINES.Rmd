...
The dataset questionnaires_coded.xlsx (Figure 2) is where all the dialogues answers are input manually, after revision. The dialogue text is reported in SuppInfo A (docs folder).

The fields that describes the answer metadata are:

-   id: identifier of the answer

-   category: domain of the BN

-   id_q: identifier of the question

-   gear: field to indicate whether the value reported is associated to a specific gear. If blank, then the value is generic

-   area: field to indicate whether the value reported is associated to a specific area If blank, then the value is generic

-   target: field to indicate whether the value reported is associated to a specific target species If blank, then the value is generic

-   short_description: short text to indicate the topic of the question
...
Data from literature are treated as described in SuppInfo B, available in the Docs section.

## Generate CPT

- Format data

The script "Step1_coding_treatments.R" is used to format the dialogues result in the proper format.

- Use data to fill CPTs

The script "Step2_BEWARE_r1_0_0.R" contains the rules to fill all the CPT of our case study. 
